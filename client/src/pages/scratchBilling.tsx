...
const ScratchBilling = () => {
  ...
  const processPaymentMutation = useMutation({
    mutationFn: async (data) => {
      const res = await apiRequest("POST", "/api/process-scratch-payment", data);
      return res.json();
    },
    onSuccess: (data) => {
  ...
  if (data.success) {
    toast({
      ...
      description: data.message || `Your ${order?.quantity} scratch(s) are ready!`,
    });
    ...
    if (competitionId) {
      setTimeout(() => {
       setLocation(`/scratch/${order?.competitionId || order?.order?.competitionId}/${orderId}`);
    ...
    onError: (error) => {
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to process scratch payment",
        variant: "destructive",
      });
    },
  });
  ...
  if (isLoading)
    return (
      <div className="flex justify-center items-center h-[70vh] bg-black text-yellow-400">
        <p>Loading scratch card order...</p>
      </div>
    );
      ...
      <main className="flex flex-col items-center justify-center flex-1 p-4">
        <div className="max-w-md w-full bg-zinc-900 border border-yellow-500/40 rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-yellow-500 to-amber-600 p-6 text-black text-center">
            <h1 className="text-2xl font-extrabold tracking-wide">SCRATCH CARD PURCHASE</h1>
            <p className="text-sm opacity-90 mt-2 font-medium">
              Complete your scratch card purchase
            </p>
          </div>
